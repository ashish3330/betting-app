...
export default function CasinoError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("[casino error boundary]", error);
  }, [error]);
    ...
    <div className="min-h-[50vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md text-center bg-surface border border-gray-800/60 rounded-xl p-8 shadow-2xl">
        ...
        <h2 className="text-lg font-bold text-white mb-2">
          Casino lobby crashed
        </h2>
        <p className="text-sm text-gray-400 mb-4">
          We couldn&apos;t load the casino games right now. Other sections of the site are still available.
        </p>
        {error?.message && (
