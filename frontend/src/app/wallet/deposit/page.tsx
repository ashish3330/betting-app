...
export default function DepositPage() {
  ...
  const [result, setResult] = useState<{ transaction_id: string; status: string; upi_link?: string } | null>(null);
  ...
  return (
    <div className="max-w-md mx-auto px-3 py-6 space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">Add money to your wallet via UPI</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {result && (
            <div className="text-xs text-profit bg-profit/10 border border-profit/20 rounded-lg px-3 py-2 space-y-1">
              <div>Deposit initiated. Transaction ID: {result.transaction_id}</div>
              <div>Status: {result.status}</div>
              {result.upi_link && (
                <a href={result.upi_link} className="underline">
                  Open UPI link
                </a>
              )}
            </div>
